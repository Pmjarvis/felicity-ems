              ...
              <div className="overflow-x-auto pb-2">
                <div className="flex gap-4" style={{ minWidth: 'max-content' }}>
                  {stats.events.map((event) => (
                    <div key={event.id} className="w-72 flex-shrink-0 border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                      <h4 className="font-semibold text-gray-900 mb-2 truncate">{event.name}</h4>
                      <div className="flex justify-between items-center text-sm">
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${event.status === 'Published' ? 'bg-green-100 text-green-800' :
